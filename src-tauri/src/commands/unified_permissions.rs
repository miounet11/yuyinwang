...
use crate::system::unified_permission_manager::{
    GuidanceInfo, PermissionReport, PermissionType, UnifiedPermissionManager,
};
...
/// 统一权限管理器状态
pub type UnifiedPermissionManagerState = Arc<Mutex<UnifiedPermissionManager>>;
...
pub async fn unified_start_permission_monitoring(
    ...
    let manager = manager.lock();
    manager.start_monitoring().await.map_err(|e| e.to_string())
}
...
pub async fn unified_quick_permission_check(
    ...
    Ok(serde_json::json!({
        ...
        "can_use_shortcuts": report.permissions.get(&PermissionType::InputMonitoring)
            .map(|s| matches!(s, crate::system::unified_permission_manager::PermissionStatus::Granted))
            .unwrap_or(false),
...
pub async fn unified_test_permissions(
    ...
    test_results.insert(
        "recording_available".to_string(),
        serde_json::Value::Bool(
            report
                .permissions
                .get(&PermissionType::Microphone)
                .map(|s| {
                    matches!(
                        s,
                        crate::system::unified_permission_manager::PermissionStatus::Granted
                    )
                })
                .unwrap_or(false),
        ),
    ...
    for (permission_type, status) in &report.permissions {
        ...
        permission_info.insert(
            ...
            serde_json::Value::String(match permission_type {
                ...
                PermissionType::ScreenRecording => "屏幕录制权限".to_string(),
                PermissionType::FullDiskAccess => "完全磁盘访问权限".to_string(),
            }),
        );
...
pub async fn unified_reset_permission_state(
    ...
    // 重置向导状态
    *manager.wizard_completed.write().unwrap() = false;
