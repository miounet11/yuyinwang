use crate::shortcuts::{insert_text_to_active_app, ShortcutManager};
use std::sync::Arc;
...
pub async fn insert_text_to_app(text: String) -> Result<(), String> {
    insert_text_to_active_app(&text).map_err(|e| e.to_string())
}
...
/// 触发语音输入测试
#[tauri::command]
pub async fn trigger_voice_input_test(app_handle: tauri::AppHandle) -> Result<(), String> {
    // 发送事件到前端触发测试
    app_handle
...
/// 显示悬浮输入窗口
#[tauri::command]
pub async fn show_floating_input(app_handle: tauri::AppHandle) -> Result<(), String> {
    // 显示悬浮输入窗口
