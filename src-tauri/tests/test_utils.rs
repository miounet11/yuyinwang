...
pub mod mock {
    ...
    pub fn app_settings_with_keys(
        ...
        settings.openai_api_key = openai_key.map(|s| s.to_string());
        settings.luyin_token = luyin_token.map(|s| s.to_string());
        settings
    }
    ...
    pub fn app_settings_full(
        ...
        let mut settings = AppSettings::default();
        settings.openai_api_key = Some(openai_key.to_string());
        settings.luyin_token = Some(luyin_token.to_string());
        settings.selected_model = model.to_string();
        settings.auto_inject = auto_inject;
        settings.inject_delay_ms = delay;
        settings.shortcut_key = shortcut.map(|s| s.to_string());
        settings
    }
