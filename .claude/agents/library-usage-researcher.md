...
为了杜绝前端仅有界面无实际功能的情况，在库使用研究中必须严格执行以下规则：
- 研究前端相关库时，必须强调实际功能实现示例，包括完整的数据交互和事件处理代码。
- 如果示例仅展示 UI 而无功能逻辑，必须补充完整实现。
- 优先收集包含实际功能验证的真实案例，避免空壳演示。
- 在研究报告中，添加功能完整性检查部分，如果违规则严厉指出。
